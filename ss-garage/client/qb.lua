...
RegisterNetEvent('ss-garage:client:SpawnVehicle', function(data)
    ...
    if coords == nil then return QBCore.Functions.Notify(Locales[SS.Config.Language]["noparkingspace"], 'error') end
    ...
    QBCore.Functions.TriggerCallback('ss-garage:qb-spawnVehicle', function(netId, properties, plate)
        ...
        exports[SS.Config.QBCore.FuelResource]:SetFuel(veh, data.fuel)
        TriggerServerEvent('ss-garage:server:qb-updateState', plate, 0)
        TriggerEvent('vehiclekeys:client:SetOwner', plate)

        if SS.Config.WarpIntoVehicle then
...
RegisterNetEvent('ss-garage:client:ParkVehicle', function(vehicle, plate, vehProps, garage)
    ...
    local fuel = exports[SS.Config.QBCore.FuelResource]:GetFuel(vehicle)
    QBCore.Functions.TriggerCallback('ss-garage:qb-parkVehicle', function(success)
        if success then
            DeleteVehicle(vehicle)
            QBCore.Functions.Notify(Locales[SS.Config.Language]["vparked"], 'success')
        else
            QBCore.Functions.Notify(Locales[SS.Config.Language]["notowned"], 'error')
        end
    end, plate, vehProps, garage, fuel, engine, body)
end)

function QBCore:OpenGarage(id)
    local garage = SS.Config.Garages[id]
    QBCore.Functions.TriggerCallback('ss-garage:qb-getvehicles', function(vehicles, errorMessage)
        if vehicles then
            SendNUIMessage({
                action = "open",
                vehicles = vehicles,
                garages = SS.Config.Garages,
                garage = id,
                locale = Locales[SS.Config.Language]["UI"],
            })
            ...
            if errorMessage then
                ...
                QBCore.Functions.Notify(Locales[SS.Config.Language]["novehicles"], 'error')
            end
        end
    end, id, garage.type, SS.Config.VehicleClass[garage.category])        
end
...
RegisterNetEvent('qb-garages:client:setHouseGarage', function(house, hasKey) -- event sent periodically from housing
    ...
    if SS.Config.Garages[formattedHouseName] then
        if hasKey and not ZoneExists(formattedHouseName) then
            CreateZone(formattedHouseName, SS.Config.Garages[formattedHouseName], 'house')
        ...
        QBCore.Functions.TriggerCallback('qb-garages:server:getHouseGarage', function(garageInfo) -- create garage if not exist
            ...
            if garageCoords then
                SS.Config.Garages[formattedHouseName] = {
                    label = house,
                ...
                TriggerServerEvent('qb-garages:server:syncGarage', SS.Config.Garages)
            end
        end, house)
    end
end)
